...
from ....core.upload.sub2api_payload import normalize_sub2api_template_config
...
class Sub2ApiTemplateConfigResponse(BaseModel):
    name_prefix: str = "GPT-"
    name_digits: int = 9
    default_concurrency: int = 1
    default_priority: int = 50
    default_rate_multiplier: float = 1
...
class Sub2ApiServiceUpdate(BaseModel):
    ...
    template_config: Optional[Dict[str, Any]] = None
    next_name_index: Optional[int] = None
...
class Sub2ApiServiceResponse(BaseModel):
    ...
    has_key: bool
    target_type: str = "sub2api"
    template_config: Sub2ApiTemplateConfigResponse
    next_name_index: int = 1
    enabled: bool
    priority: int
...
def _to_response(svc) -> Sub2ApiServiceResponse:
    return Sub2ApiServiceResponse(
        ...
        target_type=str(getattr(svc, "target_type", "sub2api") or "sub2api"),
        template_config=normalize_sub2api_template_config(getattr(svc, "template_config", None)),
        next_name_index=max(1, int(getattr(svc, "next_name_index", 1) or 1)),
        enabled=svc.enabled,
...
async def update_sub2api_service(service_id: int, request: Sub2ApiServiceUpdate):
    ...
    with get_db() as db:
        ...
        if request.template_config is not None:
            update_data["template_config"] = request.template_config
...
async def fetch_sub2api_service_groups(request: Sub2ApiFetchGroupsRequest):
    """Load Sub2API groups for a saved service or ad-hoc credentials."""
    api_url = request.api_url
    api_key = request.api_key
    ...
    if request.service_id:
        with get_db() as db:
            ...
            if not svc:
                raise HTTPException(status_code=404, detail="Sub2API 鏈嶅姟涓嶅瓨鍦?")
            api_url = api_url or svc.api_url
            api_key = api_key or svc.api_key

    if not api_url or not api_key:
        raise HTTPException(status_code=400, detail="api_url 鍜?api_key 涓嶈兘涓虹┖")

    try:
        return fetch_sub2api_groups(api_url, api_key, platform="openai")
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"鍔犺浇 Sub2API 鍒嗙粍澶辫触: {str(exc)}") from exc
